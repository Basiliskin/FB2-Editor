    ...
    return AlertDialog(
      ...
      actions: [
        TextButton(
          ...
          child: const BaseText.Text('Apply'),
        ),
      ],
    );
  }
...
class ToggleStyleButton extends StatefulWidget {
  ...
  const ToggleStyleButton({
    ...
    required this.icon,
    required this.controller,
    this.childBuilder = defaultToggleStyleButtonBuilderHandler,
    Key? key,
  }) : super(key: key);
...
Widget defaultToggleStyleButtonBuilderHandler(
  BuildContext context,
  Attribute attribute,
  IconData icon,
  bool? isToggled,
  ...
  return QuillDropdownButton<Attribute?>(
    ...
    items: [
      PopupMenuItem(
        ...
        height: iconSize * 1.77,
        child: BaseText.Text(_valueToText[Attribute.header]!, style: style),
      ),
      PopupMenuItem(
        value: Attribute.h1,
        height: iconSize * 1.77,
        child: BaseText.Text(_valueToText[Attribute.h1]!, style: style),
      ),
      PopupMenuItem(
        value: Attribute.h2,
        height: iconSize * 1.77,
        child: BaseText.Text(_valueToText[Attribute.h2]!, style: style),
      ),
      PopupMenuItem(
        value: Attribute.h3,
        height: iconSize * 1.77,
        child: BaseText.Text(_valueToText[Attribute.h3]!, style: style),
      ),
    ],
...
class _ColorButtonState extends State<ColorButton> {
  ...
  Widget build(BuildContext context) {
    ...
    Color? iconColor = _isToggledColor && !widget.background && !_isWhite
        ? ColorUtils.stringToColor(_selectionStyle.attributes['color']!.value)
        : theme.iconTheme.color;

    var iconColorBackground =
        _isToggledBackground && widget.background && !_isWhitebackground
            ? ColorUtils.stringToColor(
                _selectionStyle.attributes['background']!.value)
            : theme.iconTheme.color;

    Color fillColor = _isToggledColor && !widget.background && _isWhite
        ? ColorUtils.stringToColor('#ffffff')
        : theme.canvasColor;
  ...
  void _showColorPicker() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
          title: const BaseText.Text('Select Color'),
...
class CustomQuillToolbar extends StatefulWidget implements PreferredSizeWidget {
  ...
  const CustomQuillToolbar({required this.children, Key? key})
      : super(key: key);
  ...
  factory CustomQuillToolbar.basic(
    ...
    return CustomQuillToolbar(key: key, children: children);
  }
