...
class EasyFormState extends State<EasyForm> {
  ...
  void _fieldDidChange(EasyFormFieldState<dynamic>? field) {
    ...
    if (widget.rebuildOnChange) _forceRebuild();
  }
  ...
  Future<bool> save([bool ignoreValidate = false]) async {
    if (!ignoreValidate && !validate()) return false;
  ...
  EasyFormFieldState<dynamic>? _validate([bool isInteraction = false]) {
  ...
  void initState() {
    ...
    widget.triggerSave!.listen((event) {
      try {
        print('EasyFormState.triggerSave');
        save();
      } catch (e) {
        print(e);
      }
